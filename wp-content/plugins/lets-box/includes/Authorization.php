...
class Authorization
{
    ...
    {
        return LETSBOX_CACHEDIR.'/'.$this->_token_name;
    }
    ...
    public function get_access_token()
    {
        ...
        $raw_token_data = Helpers::decrypt($this->update_from_plain_token($token));
        ...
        $token->setExpiresIn($token_data['expires_in']);
        $token->setTokenType($token_data['token_type']);
        $token->setRefreshToken($token_data['refresh_token']);
        $token->setCreated($token_data['created']);
    ...
    public function set_access_token($_access_token)
    {
        ...
        if (false !== ($timestamp = wp_next_scheduled('letsbox_lost_authorisation_notification', ['account_id' => $this->_account_id]))) {
            wp_unschedule_event($timestamp, 'letsbox_lost_authorisation_notification', ['account_id' => $this->_account_id]);
        }
