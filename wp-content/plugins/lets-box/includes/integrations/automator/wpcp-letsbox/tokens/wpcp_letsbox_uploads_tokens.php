...
/**
 * Class WPCP_LETSBOX_UPLOADS_TOKENS.
 */
class WPCP_LETSBOX_UPLOADS_TOKENS
{
    /**
    ...
    public function __construct()
    {
        ...
        foreach (self::TOKENS_TRIGGERS as $trigger_code) {
            add_filter('automator_maybe_trigger_wpcp-letsbox_'.strtolower($trigger_code).'_tokens', [$this, 'register_tokens'], 20, 2);
        }
    ...
    public function register_tokens($tokens = [], $args = [])
    {
        ...
        foreach ($tokens_collection as $token) {
            $tokens[] = [
                ...
                'tokenType' => 'text',
                'tokenIdentifier' => strtoupper('WPCP_LETSBOX_'.$token['id']),
            ];
        }
    ...
    public function save_token_data($args, $trigger)
    {
        ...
        $total_file_size = \TheLion\LetsBox\Helpers::bytes_to_size_1024(array_sum($sizes_in_bytes));
        ...
        $upload_data_arr = [
            'data' => [
                ...
                'upload_folder_url' => 'https://app.box.com/'.($first_cached_node->get_entry()->is_dir() ? 'folder' : 'file').'/'.$first_cached_node->get_id(),
                'owners' => $linked_users,
                'owner_names' => array_column($linked_users, 'display_name'),
                'owner_emailaddresses' => array_column($linked_users, 'user_email'),
            ],
            'user_id' => get_current_user_id(),
            'page' => \TheLion\LetsBox\Helpers::get_page_url(),
        ];

        $upload_data = wp_json_encode($upload_data_arr);

        Automator()->db->token->save('WPCP_LETSBOX_UPLOADS_DATA', $upload_data, $args['trigger_entry']);
    }
    ...
    public function parse_tokens($value, $pieces, $recipe_id, $trigger_data, $user_id, $replace_args)
    {
        ...
        $upload_data = json_decode(Automator()->db->token->get('WPCP_LETSBOX_UPLOADS_DATA', $replace_args), true);
        ...
        switch ($pieces[2]) {
            ...
            case 'uploaded_files_plain':
                foreach ($upload_data['data']['entries'] as $entry) {
                    $value .= "{$entry['name']} (".\TheLion\LetsBox\Helpers::bytes_to_size_1024($entry['size']).")\r\n";
                }
    ...
    <tr style="<?php echo ($current % 2) ? 'background: #fafafa;' : ''; ?> height: 26px;">
        <td style="width:20px;padding-right:10px;padding-left:5px;border:none;">
            <img alt="" height="16" src="<?php echo \TheLion\LetsBox\Helpers::get_default_icon($entry['mimetype']); ?>" style="border:0;display:block;outline:none;text-decoration:none;height:auto;width:16px;max-width:16px;" width="16">
        </td>
        <td style="padding-right:10px;border:none;">
            <a href="<?php echo urldecode($entry['preview_link']); ?>" target="_blank"><?php echo basename($entry['name']).' ('.\TheLion\LetsBox\Helpers::bytes_to_size_1024($entry['size']).')'; ?></a>
