...
/**
 * Class WPCP_LETSBOX_EVENT_TOKENS.
 */
class WPCP_LETSBOX_EVENT_TOKENS
{
    /**
    ...
    public function save_token_data($args, $trigger)
    {
        ...
        // Custom values
        $entry_arr['size'] = \TheLion\LetsBox\Helpers::bytes_to_size_1024($cached_node->get_entry()->get_size());
        $entry_arr['icon'] = $cached_node->get_entry()->get_icon();
        $entry_arr['thumbnail'] = $cached_node->get_entry()->get_thumbnail_large();
        $entry_arr['thumbnail'] = \TheLion\LetsBox\Client::instance()->get_thumbnail($cached_node->get_entry(), true, 500, 500, false, false);
        $entry_arr['preview_url'] = 'https://app.box.com/'.($cached_node->get_entry()->is_dir() ? 'folder' : 'file').'/'.$cached_node->get_id();
        $entry_arr['download_url'] = null;

        // Folder owners
        $linked_users = $cached_node->get_linked_users();
        $current_account = \TheLion\LetsBox\App::get_current_account();
        ...
        Automator()->db->token->save('WPCP_LETSBOX_EVENT_DATA', $event_data_str, $args['trigger_entry']);
    }
    ...
    public function parse_tokens($value, $pieces, $recipe_id, $trigger_data, $user_id, $replace_args)
    {
        ...
        $event_data = json_decode(Automator()->db->token->get('WPCP_LETSBOX_EVENT_DATA', $replace_args), true);
