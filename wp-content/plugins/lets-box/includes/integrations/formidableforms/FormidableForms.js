(function ($) {
    ...
    function openShortcodeBuilder(shortcode) {
        ...
        var modalbuttons = '';
        var modalheader = $(
            '<a tabindex="0" class="close-button" title="" onclick="window.modal_action.close();"><i class="eva eva-close eva-lg" aria-hidden="true"></i></a></div>'
        );
        var modalbody = $('<div class="letsbox-modal-body" tabindex="0" style="display:none;padding:0!important;"></div>');
        var modaldialog = $(
            '<div id="letsbox-modal-action" class="LetsBox letsbox-modal letsbox-modal80 light"><div class="modal-dialog"><div class="modal-content" style="padding:40px 0 0 0!important"><div class="loading"><div class="loader-beat"></div></div></div></div></div>'
        );
        ...
        var $iframe_template = $(
            "<iframe src='" +
                window.ajaxurl +
                '?action=letsbox-getpopup&type=shortcodebuilder&asuploadbox=1&' +
                query +
                "' width='100%' height='600' tabindex='-1' frameborder='0'></iframe>"
        );
