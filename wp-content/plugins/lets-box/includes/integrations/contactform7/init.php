...
class ContactForm
{
    ...
    public function __construct(\TheLion\LetsBox\Main $_main)
    {
        ...
        add_filter('wpcf7_mail_tag_replaced_letsbox*', [$this, 'set_email_tag'], 999, 4);
        add_filter('wpcf7_mail_tag_replaced_letsbox', [$this, 'set_email_tag'], 999, 4);
    ...
    {
        wp_register_script('LetsBox.ContactForm7', plugins_url('ContactForm7.js', __FILE__), ['jquery'], LETSBOX_VERSION, true);
        wp_enqueue_script('LetsBox.ContactForm7');
    }
    ...
    public function add_tag_generator()
    {
        if (class_exists('WPCF7_TagGenerator')) {
            ...
            $tag_generator->add('letsbox', 'Lets-Box', [$this, 'tag_generator_body']);
        }
    }
    ...
    public function shortcode_handler($tag)
    {
        ...
        if ($required) {
            add_filter('letsbox_shortcode_set_options', [$this, 'set_required_shortcode'], 10, 3);
        }

        // Shortcode
        $shortcode = base64_decode(urldecode($tag->get_option('shortcode', '', true)));
        $return = apply_filters('letsbox-wpcf7-render-shortcode', do_shortcode($shortcode));
