...
class Events
{
    ...
    {
        add_action('wp_ajax_nopriv_letsbox-event-stats', [$this, 'get_stats']);
        add_action('wp_ajax_letsbox-event-stats', [$this, 'get_stats']);

        add_action('wp_ajax_nopriv_letsbox-event-log', [$this, 'log_ajax_event']);
        add_action('wp_ajax_letsbox-event-log', [$this, 'log_ajax_event']);

        add_action('letsbox_log_event', [$this, 'log_event'], 10, 3);

        add_action('letsbox_send_event_summary', [$this, 'send_event_summary']);
    }
    ...
    public function get_activities($period = null)
    {
        ...
        foreach ($data as $day) {
            if (!isset($raw_chart_data['datasets'][$day['type']])) {
                ...
                $hide_on_init = !in_array($day['type'], ['letsbox_previewed_entry', 'letsbox_downloaded_entry', 'letsbox_streamed_entry']);
    ...
    public function get_top_downloads($period = null)
    {
        ...
        $where = "`plugin` = 'lets-box' AND `type` IN ('letsbox_downloaded_entry')";
    ...
    public function get_latest_uploads($period = null)
    {
        ...
        $where = "`plugin` = 'lets-box' AND `type` IN ('letsbox_uploaded_entry')";
    ...
    public function get_event_type($type)
    {
        switch ($type) {
            ...
            case 'letsbox_created_link_to_entry':
            ...
            case 'letsbox_sent_notification':
    ...
    public function send_event_summary()
    {
        ...
        $filter_events = apply_filters('letsbox_events_set_summary_events', [
        ...
        $max_top_downloads = apply_filters('letsbox_events_set_summary_top_downloads', 10, $this);
        $max_events = apply_filters('letsbox_events_set_summary_max_events', 25, $this);
        ...
        $interval = apply_filters('letsbox_events_set_summary_period', $interval, $this);
...
class Event_DB_Model
{
    ...
    public static function table()
    {
        ...
        return $wpdb->prefix.'wp_cloudplugins_log_letsbox';
    }
