...
(function () {
    ...
    tinymce.PluginManager.add('letsbox', function (ed, url) {
        ...
        ed.on('PostProcess', function (ed) {
            if (ed.get) ed.content = get_lb_shortcode(ed.content);
        });
    });
    ...
    function do_lb_shortcode(co, url) {
        return co.replace(/\[letsbox([^\]]*)\]/g, function (a, b) {
            return (
                '<img src="' +
                url +
                '/../../css/images/transparant.png" class="wp_lb_shortcode mceItem" title="Lets-Box" data-mce-placeholder="1" data-code="' +
                toBinary(b) +
                '"/>'
            );
        });
    ...
    function get_lb_shortcode(co) {
        ...
        return co.replace(/(?:<p[^>]*>)*(<img[^>]+>)(?:<\/p>)*/g, function (a, im) {
            ...
            if (cls.indexOf('wp_lb_shortcode') != -1) return '<p>[letsbox ' + tinymce.trim(fromBinary(getAttr(im, 'data-code'))) + ']</p>';
