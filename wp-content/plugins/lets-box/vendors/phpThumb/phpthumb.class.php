...
$predefined_IMAGETYPE_constants = [
    'IMAGETYPE_GIF' => 1,
    'IMAGETYPE_JPEG' => 2,
    'IMAGETYPE_PNG' => 3,
    'IMAGETYPE_SWF' => 4,
...
class phpthumb
{
    ...
    public $src;     // SouRCe filename
    public $new;     // NEW image (phpThumb.php only)
    public $w;     // Width
    public $h;     // Height
    public $wp;     // Width  (Portrait Images Only)
    public $hp;     // Height (Portrait Images Only)
    public $wl;     // Width  (Landscape Images Only)
    public $hl;     // Height (Landscape Images Only)
    public $ws;     // Width  (Square Images Only)
    public $hs;     // Height (Square Images Only)
    public $f;     // output image Format
    public $q = 75;       // jpeg output Quality
    public $sx;     // Source crop top-left X position
    public $sy;     // Source crop top-left Y position
    public $sw;     // Source crop Width
    public $sh;     // Source crop Height
    public $zc;     // Zoom Crop
    public $ica;     // Image Crop Auto
    public $bc;     // Border Color
    public $bg;     // BackGround color
    public $fltr = [];  // FiLTeRs
    public $goto;     // GO TO url after processing
    public $err;     // default ERRor image filename
    public $xto;     // extract eXif Thumbnail Only
    public $ra;     // Rotate by Angle
    public $ar;     // Auto Rotate
    public $aoe;     // Allow Output Enlargement
    public $far;     // Fixed Aspect Ratio
    public $iar;     // Ignore Aspect Ratio
    public $maxb;     // MAXimum Bytes
    public $down;     // DOWNload thumbnail filename
    public $md5s;     // MD5 hash of Source image
    public $sfn = 0;        // Source Frame Number
    public $dpi = 150;      // Dots Per Inch for vector source formats
    public $sia;     // Save Image As filename

    public $file;     // >>>deprecated, DO NOT USE, will be removed in future versions<<<

    public $phpThumbDebug;
    // END PARAMETERS
    ...
    public $config_cache_directory;
    public $config_cache_directory_depth = 0;
    public $config_cache_disable_warning = true;
    public $config_cache_source_enabled = false;
    public $config_cache_source_directory;
    public $config_temp_directory;
    public $config_document_root;
    ...
    // * MySQL
    public $config_mysql_extension;
    public $config_mysql_query;
    public $config_mysql_hostname;
    public $config_mysql_username;
    public $config_mysql_password;
    public $config_mysql_database;

    // * Security
    public $config_high_security_enabled = true;
    public $config_high_security_password;
    ...
    public $rawImageData;
    public $IMresizedData;
    public $outputImageData;
    ...
    public $thumbnailCropX;
    public $thumbnailCropY;
    public $thumbnailCropW;
    public $thumbnailCropH;

    public $exif_thumbnail_width;
    public $exif_thumbnail_height;
    public $exif_thumbnail_type;
    public $exif_thumbnail_data;
    public $exif_raw_data;

    public $thumbnail_width;
    public $thumbnail_height;
    public $thumbnail_image_width;
    public $thumbnail_image_height;

    public $tempFilesToDelete = [];
    public $cache_filename;

    public $AlphaCapableFormats = ['png', 'ico', 'gif', 'webp', 'avif'];
    ...
    public function phpThumb()
    {
        ...
        foreach ([ini_get('memory_limit'), get_cfg_var('memory_limit')] as $php_config_memory_limit) {
            if (!empty($php_config_memory_limit)) {
                if ('G' == strtoupper($php_config_memory_limit[strlen($php_config_memory_limit) - 1])) { // PHP memory limit expressed in Gigabytes
                    $php_config_memory_limit = (int) substr($php_config_memory_limit, 0, -1) * 1073741824;
                } elseif ('M' == strtoupper($php_config_memory_limit[strlen($php_config_memory_limit) - 1])) { // PHP memory limit expressed in Megabytes
        ...
        $this->iswindows = (bool) ('WIN' == strtoupper(substr(PHP_OS, 0, 3)));
        ...
        if (($this->config_cache_maxage > 0) || ($this->config_cache_maxsize > 0) || ($this->config_cache_maxfiles > 0)) {
            ...
            foreach ($CacheDirOldFilesSize as $fullfilename => $filesize) {
                ...
                if ((0 == $filesize) && ($CacheDirOldFilesAge[$fullfilename] < $cutofftime)) {
                    ...
                    if (@unlink($fullfilename)) {
                        ...
                        unset($CacheDirOldFilesSize[$fullfilename], $CacheDirOldFilesAge[$fullfilename]);
                    }
                }
            }
            ...
            if ($this->config_cache_maxfiles > 0) {
                ...
                foreach ($DeletedKeys['maxfiles'] as $fullfilename) {
                    unset($CacheDirOldFilesAge[$fullfilename], $CacheDirOldFilesSize[$fullfilename]);
                }
            }
    ...
    public function setOutputFormat()
    {
        ...
        if ($this->ImageMagickVersion()) {
            $IMformats = ['jpeg', 'png', 'gif', 'bmp', 'ico', 'wbmp', 'webp', 'avif'];
    ...
    public function realPathSafe($filename)
    {
        ...
        if (phpthumb_functions::is_windows()) {
            ...
            $isAlreadyAbsoluteFilename = (DIRECTORY_SEPARATOR == $newfilename[0]);  // /path/filename.ext
        }
    ...
    public function ResolveFilenameToAbsolute($filename)
    {
        ...
        if ($this->iswindows && isset($filename[1]) && (':' == $filename[1])) {
        ...
        if ($commandline) {
            ...
            if ($IMtempfilename = $this->phpThumb_tempnam()) {
                ...
                if (null !== $this->dpi && $this->ImageMagickSwitchAvailable('density')) {
                    ...
                    if (is_array($getimagesize) && isset($getimagesize[2]) && (IMAGETYPE_PNG == $getimagesize[2])) {
                ...
                if (is_array($getimagesize)) {
                    ...
                    if (!preg_match('#('.implode('|', $this->AlphaCapableFormats).')#i', $outputFormat)) {
                        ...
                        if (IMAGETYPE_PNG == $getimagesize[2] && !$this->bg) {
    ...
    public function SourceImageToGD()
    {
        ...
        if (!$this->gdimg_source) {
            ...
            if (!$this->gdimg_source) {
                ...
                if ("\xD7\xCD\xC6\x9A" == $HeaderFourBytes) { // WMF
                    return $this->ErrorImage($this->ImageMagickVersion() ? 'ImageMagick failed on WMF source conversion' : 'ImageMagick is unavailable and phpThumb() does not support WMF source images without it');
                }
                if ('%PDF' == $HeaderFourBytes) { // "%PDF"
                ...
                if ("\xFF\xD8\xFF" == substr($HeaderFourBytes, 0, 3)) { // JPEG
                ...
                if ('GIF' == substr($HeaderFourBytes, 0, 3)) { // GIF
    ...
    public function ImageCreateFromStringReplacement(&$RawImageData, $DieOnErrors = false)
    {
        ...
        if ($tempnam = $this->phpThumb_tempnam()) {
            if ($fp_tempnam = @fopen($tempnam, 'wb')) {
                ...
                fclose($fp_tempnam);
                @chmod($tempnam, $this->getParameter('config_file_create_mask'));
                if (('imagecreatefromgif' == $ICFSreplacementFunctionName) && !function_exists($ICFSreplacementFunctionName)) {
        ...
        if (!is_null($this->ica)) {
            ...
            if (function_exists('imagecropauto')) { // (PHP 5 >= 5.5.0, PHP 7)
                ...
                if (preg_match('#^(([0-4])|(5)\\|(0?\\.?[0-9]+)\\|([0-9A-F]{6}))$#i', $this->ica, $matches)) {
                    ...
                    if (false !== $cropped) {                 // in case a new image resource was returned
