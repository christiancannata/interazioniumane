...
namespace Christiancannata\Woofic;
...
class WooficSender
{
    ...
    public function createInvoice(\WC_Order $order)
    {
        ...
        if (!$ficPaymentMethod) {
            die("Errore metodo di pagamento non sincronizzato");
        }

        //check payment account
        $ficPaymentAccountId = get_option('fic_' . $order->get_payment_method() . '_payment_account');
        if (!$ficPaymentAccountId) {
            die("Errore conto di pagamento non sincronizzato");
        }
        ...
        if (!$companyName) {
            $companyName = $order->get_billing_last_name() . " " . $order->get_billing_first_name();
        }
        ...
        foreach ($order->get_items() as $item) {
            ...
            if ($selectedTax) {
                ...
                $ficVatCode = reset($ficTaxRate)->getId();
                ...
                [
                    new IssuedDocumentPaymentsListItem(
                        [
                            ...
                            "payment_account" => new PaymentAccount(['id' => intval($ficPaymentAccountId)])
                        ]
                    )
                ]
            );
...
//	$get_new_issued_document_totals_request->setData( $invoice );
    ...
    public function getToken()
    {
        ...
        if ($code) {
            ...
            $token = get_option('woofic_token', null);

            $expireDate = get_option('woofic_token_expire', null);

            if (!$expireDate || new \DateTime() > $expireDate) {
                ...
                $token = $result_decoded;

                $date = new \DateTime();
                $date->add(new \DateInterval('PT' . (intval($result_decoded['expires_in']) - 40) . "S"));

                update_option('woofic_token_expire', $date);
            }
